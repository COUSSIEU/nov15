...
public class ChampEffectifGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all champEffectifs").get("/api/champ-effectifs").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new champEffectif")
                        .post("/api/champ-effectifs")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"nom\": \"SAMPLE_TEXT\"" + ", \"valeur\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_champEffectif_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created champEffectif").get("${new_champEffectif_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created champEffectif").delete("${new_champEffectif_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ChampEffectif entity").exec(scn);
